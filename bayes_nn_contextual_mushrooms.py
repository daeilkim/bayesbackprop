...
def update_nn(init_var_params, batch_data, batch_labels, iteration):
    ...
    variational_params = adam(gradient, init_var_params, step_size=0.1, num_iters=10)
...
if __name__ == '__main__':
    ...
    num_weights, predictions, logprob = \
        make_nn_funs(layer_sizes=[F, num_units, num_units, 1],
                     L2_reg=0.01,
                     noise_variance=0.01,
                     nonlinearity=relu)
    ...
    for step in range(num_steps):
        ...
        if len(experience) > batch_size*2:
            for ix in xrange(5):
                ...
                variational_params, objective = update_nn(variational_params, batch_data, batch_labels, ix)


        if (step%100 == 0) and (step > batch_size*2):
            ...
            print ('Lower Bound: %f' % (-objective(variational_params, step)))
            prev_regret = cumulative_regret
            plt.cla()
            var_mean, var_log_std = variational_params[:num_weights], variational_params[num_weights:]
